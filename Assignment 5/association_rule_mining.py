...
# SPECIFICATION:    Read file to find strong rules related to supermarket products
...
for i in range(len(rules)):
    ...
    for j in encoded_vals:
        ...
        for key in arr:
            del copy[key]
        if set(copy.keys()).issuperset(temp):
            supportCount += 1
    prior = supportCount / len(encoded_vals)
    print("Prior:", prior)
    print("Gain in confidence:", 100 * (rules["confidence"][i] - prior) / prior)
